...
Partition::Partition(size_t num_of_states, const StateBlocks& partition) {
    ...
    if(num_of_states) [[likely]] {
        states_info_.reserve(num_of_states);
    ...
    // initialization of the states_info_ vector
    for(size_t i = 0; i < num_of_states; ++i) {
        states_info_.emplace_back(i, i, 0, 0, *this);
    }
    ...
    for(size_t block_idx = 0; block_idx < num_of_blocks; ++block_idx) {
        ...
        for(auto state_idx : partition[block_idx]) {
            ...
            states_info_[state_idx].block_item_idx_ = block_item_idx;
            states_info_[state_idx].block_idx_ = block_idx;
            states_info_[state_idx].node_idx_ = block_idx;
        ...
        nodes_.emplace_back(node_idx, states_info_[first].block_item_idx_, states_info_[last].block_item_idx_, std::nullopt, *this);
    ...
    for(size_t state_idx = 0; state_idx < num_of_states; ++state_idx) {
        ...
        states_[state_idx] = block_item_idx;
        states_info_[state_idx].block_item_idx_ = block_item_idx;
        states_info_[state_idx].block_idx_ = num_of_blocks - 1;
        states_info_[state_idx].node_idx_ = num_of_blocks - 1;
    ...
    if(!all_states_used) { 
        nodes_.emplace_back(nodes_.size(), states_info_[first].block_item_idx_, states_info_[last].block_item_idx_, std::nullopt,
            *this);
        blocks_.emplace_back(num_of_blocks-1, num_of_blocks-1, *this);
    }

    marked_ = GenerationSparseSet<size_t>(num_of_states);
    touched_ = SparseMultiset<size_t>(num_of_states);
}
...
Partition::Partition(Partition&& other)
    : Partition(static_cast<const Partition&>(other))
{}

Partition& Partition::operator=(Partition&& other) {
    return operator=(static_cast<const Partition&>(other));
}

/**
* @brief returns a StateInfo corresponding to the given index
* @param[in] state_info_idx index of the BlockItem
* @return corresponding StateInfo
*/
const Partition::StateInfo& Partition::get_state_info(
    size_t state_info_idx) const {

    assert(state_info_idx < num_of_states() &&
           "Nonexisting state index used.");    
    return states_info_[state_info_idx];
}
...
bool Partition::in_same_block(State first, State second) const {
    ...
    return get_block_idx_of_state(first) == get_block_idx_of_state(second);
}
...
bool Partition::in_same_block(const std::vector<size_t>& state_idxs) const {
    ...
    size_t block_idx = get_block_idx_of_state(state_idxs.front());
    for(size_t state_idx : state_idxs) {
        assert(state_idx < states_info_.size() && "The given state does not exist.");
        if(get_block_idx_of_state(state_idx) != block_idx) { return false; }
    }
    return true;
}
...
std::vector<size_t> Partition::states_in_same_block(size_t state_idx) const {
    ...
    for(const BlockItem& block_item : (*this)[state_idx].block()) {
        result.push_back(block_item.state());
    }
...
std::vector<SplitPair> Partition::split_blocks(
    GenerationSparseSet<size_t>& marked) {
    ...
    // marked and keeps number of states of each block which have been marked
    ...
    for(size_t i : marked) {
        assert(i < states_info_.size() && "The given state does not exist.");
        ++used_blocks[get_block_idx_of_state(i)];
    }    
    ...
    for(size_t i = 0; i < old_blocks_size; ++i) {
        ...
        size_t num_of_nodes = nodes_.size();
        ...
        bool repr_marked = marked[node.first().state()];
        ...
        do {
            ...
            while(repr_marked ^ (!marked[get_block_item(iter_first).state()])) {
                ...
                states_info_[block_items_[iter_first].state_].node_idx_ = num_of_nodes;
                ++iter_first;
            }
            while(repr_marked ^ marked[get_block_item(iter_last).state()]) {
                ...
                states_info_[block_items_[iter_last].state_].block_item_idx_ = iter_last;
                states_info_[block_items_[iter_last].state_].block_idx_ = new_block_idx;
                states_info_[block_items_[iter_last].state_].node_idx_ = num_of_nodes + 1;
                --iter_last;
            }
            ...
            states_info_[block_items_[iter_first].state_].block_item_idx_ = iter_first;
            states_info_[block_items_[iter_last].state_].block_item_idx_ = iter_last;
        ...
        nodes_.emplace_back(nodes_.size(), first_idx, iter_last, node.idx(), *this);
        nodes_.emplace_back(nodes_.size(), iter_first, last_idx, node.idx(), *this);
        nodes_[node.idx()].set_children(nodes_.size() - 2, nodes_.size() - 1);
...
Partition& Partition::operator=(const Partition& other) {
    ...
    for(size_t i = 0; i < states_num; ++i) {
        const StateInfo& si = other.get_state_info(i);
        states_info_.emplace_back(
            StateInfo(si.idx_, si.block_item_idx_, si.block_idx_, si.node_idx_, *this));
    }
    for(size_t i = 0; i < states_num; ++i) {
    ...
    for(size_t i = 0; i < nodes_num; ++i) {
        ...
        nodes_.emplace_back(Node(n.idx_, n.first_, n.last_, n.parent_, *this));
        nodes_.back().left_child_ = n.left_child_;
        nodes_.back().right_child_ = n.right_child_;
    }
    return *this;
}
...
const Partition::BlockItem& Partition::operator[](State state) const {
    assert(state < states_info_.size() && "The given state does not exist.");
    return block_items_[states_info_[state].block_item_idx_];
}

/** Partition equality operator. Two partitions are considered to be equal
* iff they have the same carrier set (the same number of states), the
* same cardinality (the same number of blocks) and if each pair of states either
* shares or does not share the same block in both partitions at the same time.
* The nodes and the indices of block items and blocks do not matter in context
* of partition equality.
* @brief checks whether two partitions are equal
* @param[in] other partition
* @return true iff the given partitions are equal
*/
bool Partition::operator==(const Partition& other) const {
    if(num_of_states() != other.num_of_states()) { return false; }
    if(num_of_blocks() != other.num_of_blocks()) { return false; }
    for(const Block& block : blocks_) {
        State repr_state = block.first().state();
        for(const BlockItem& block_item : block) {
            if(!other.in_same_block(repr_state, block_item.state())) {
                return false;
            }
        }
    }
...
std::ostream& operator<<(std::ostream& os, const Partition& p) {
    ...
    result += "STATE INFO:\n";
    result += "state -> block_item_idx block_idx node_idx\n";
    for(const Partition::StateInfo& state_info : p.states_info_) {
        result += std::to_string(state_info.idx()) + " -> " +
            std::to_string(state_info.block_item_idx()) + " " +
            std::to_string(state_info.block_idx()) + " " +
            std::to_string(state_info.node_idx()) + "\n";
    ...
    for(const Partition::Node& node : p.nodes_) {
        ...
        if(node.parent()) {
            result += "[p: " + std::to_string(*node.parent()) + ", ";
        } else {
            result += "[p: X, ";
        }
        if(node.left_child()) {
            result += "l: " + std::to_string(*node.left_child()) + ", ";
            result += "r: " + std::to_string(*node.right_child()) + "]\n";
        } else {
            result += "l: X, r: X]\n";
        }
    }
...
* O(1)
*/
void Partition::mark(State state) {
    assert(state < num_of_states());
    if(!marked_.insert(state)) { return; }
    touched_.insert(states_info_[state].block_idx_);    
}
...
* O(1)
*/
void Partition::clear_markings(void) {
    marked_.clear();
    touched_.clear();
}
...
* O(1)
*/
bool Partition::is_marked(State state) const { return marked_[state]; }
...
* O(1)
*/
bool Partition::is_touched(size_t block_idx) const { return touched_[block_idx]; }
...
* O(1)
*/
bool Partition::is_fully_marked(size_t block_idx) const { 
    return get_size_of_block_idx(block_idx) == touched_.occurrences(block_idx);
}
...
std::vector<SplitPair> Partition::split_blocks(
    ...
    // iterating through the marked states to fill used_blocks vector
    for(auto item : get_block(block_idx)) {
        if(marked[item.state()]) {
            ++used_block;
        }
    }  
    
    size_t new_block_idx;
    new_block_idx = blocks_.size();

    // if no state of the given block has been marked, it won't be split
    if(!used_block) { return split; }
    
    // looking for the subvector of BlockItems which forms processed
    // block and computing its size
    Node node = get_block(block_idx).node();
    size_t iter_first = node.first().idx();
    size_t iter_last = node.last().idx();
    size_t block_size = node.size();
    size_t num_of_nodes = nodes_.size();
    ...
    // if all states of the processed block have been marked, the block
    // won't be split
    if(used_block >= block_size) { return split; }
    ...
    // split blocks has to refer to the new nodes
    blocks_[block_idx].node_idx_ = nodes_.size() - 2;
    blocks_.emplace_back(new_block_idx, nodes_.size() - 1, *this);
    blocks_.back().idx_ = new_block_idx;
    ...
    // indices of components of split block and about the node which
    // correspond to the block which has been split        
    split.emplace_back(block_idx, new_block_idx, node.idx());
...
* O(1)
*/
std::vector<SplitPair> Partition::split_blocks_inner(size_t block_idx) {
    
    // the vector which will be returned as the result
    std::vector<SplitPair> split{};
    
    // if there is no marked state, no block could be split
    if(marked_.empty()) [[unlikely]] { return split; }  
    
    size_t new_block_idx = blocks_.size();

    // if no state of the given block has been marked, it won't be split
    if(!touched_[block_idx]) { return split; }
    // looking for the subvector of BlockItems which forms processed
    // block and computing its size
    Node node = get_block(block_idx).node();
    size_t iter_first = node.first().idx();
    ...
    if(touched_.occurrences(block_idx) >= block_size) {
        return split;
    }
   
    // choosing the strategy of swapping BlocksItems such that
    // the representant of split block keeps its position        
    bool repr_marked = marked_[node.first().state()];
    ...
    do {
        ...
        // we choose the swapping strategy using XOR operation
        while(repr_marked ^ (!marked_[get_block_item(iter_first).state()])) {
            
            // this visited state will be part of the former block
            states_info_[block_items_[iter_first].state_].node_idx_ = num_of_nodes;
            ++iter_first;
        }
        while(repr_marked ^ marked_[get_block_item(iter_last).state()]) {
    ...
    if(repr_marked) {
        touched_.set_count(block_idx, get_size_of_block_idx(block_idx));
    }
    else {
        touched_.insert(new_block_idx);
        touched_.set_count(new_block_idx, get_size_of_block_idx(new_block_idx));
        touched_.erase(block_idx);
    }
